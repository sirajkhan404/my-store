...
const Orders = () => {
    ...
    return (
        <div style={{ padding: '28px 24px', fontFamily: "'Inter', sans-serif", minHeight: '100%', background: '#f8fafc' }}>
            ...
            {/* ── Header ── */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12, animation: 'ord-in 0.4s ease' }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: '#0f172a', letterSpacing: -0.5 }}>🛒 Orders Management</h1>
                    <p style={{ margin: '4px 0 0', fontSize: 13, color: '#94a3b8' }}>Track and manage all customer orders</p>
                </div>
                <button onClick={getDocuments} style={{ padding: '9px 18px', borderRadius: 10, border: '1.5px solid #e2e8f0', background: '#fff', color: '#6366f1', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7 }}>
                    🔄 Refresh
                </button>
            </div>

            {/* ── Stat Cards ── */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 14, marginBottom: 24, animation: 'ord-in 0.4s ease 0.05s both' }}>
                {[
                    ...
                    <div key={i} style={{ background: '#fff', borderRadius: 16, padding: '16px 18px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: 19, fontWeight: 800, color: '#0f172a', lineHeight: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.value}</div>
            ...
            {/* ── Filters ── */}
            <div style={{ background: '#fff', borderRadius: 16, padding: '14px 18px', marginBottom: 18, boxShadow: '0 2px 10px rgba(0,0,0,0.05)', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', animation: 'ord-in 0.4s ease 0.1s both' }}>
                ...
                <div style={{ marginLeft: 'auto', fontSize: 12, color: '#94a3b8', fontWeight: 500 }}>{filtered.length} of {documents.length} orders</div>
            </div>

            {/* ── Orders List ── */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, animation: 'ord-in 0.4s ease 0.15s both' }}>

                {loading && (
                    <div style={{ background: '#fff', borderRadius: 16, padding: 56, textAlign: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                ...
                {!loading && filtered.map((order, idx) => {
                    ...
                    return (
                        <div key={order.id} style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 10px rgba(0,0,0,0.05)', overflow: 'hidden', border: `1px solid ${isExpanded ? '#e0e7ff' : '#f1f5f9'}`, transition: 'border 0.2s' }}>

                            {/* ── Main Row ── */}
                            <div className="ord-row" onClick={() => setExpandedRow(isExpanded ? null : order.id)}
                                style={{ display: 'grid', gridTemplateColumns: '28px 1.5fr 2fr 1.2fr 1.2fr 1fr 1fr auto', gap: 0, padding: '14px 20px', alignItems: 'center', background: isExpanded ? '#f8f7ff' : '#fff' }}>
                                ...
                                {/* Customer */}
                                <div>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{order.shippingAddress?.fullName || '—'}</div>
                                    <div style={{ fontSize: 11, color: '#94a3b8' }}>{order.shippingAddress?.phone}</div>
                                    <div style={{ fontSize: 11, color: '#94a3b8' }}>{order.shippingAddress?.city}</div>
                                </div>
                            ...
                            {isExpanded && (
                                <div style={{ padding: '0 20px 18px', borderTop: '1px solid #f1f5f9' }}>
                                    <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.8, margin: '14px 0 10px' }}>Products in this order</div>
            ...
            {isModalOpen && orderToEdit && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,12,41,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(4px)' }}
                    ...
                    <div style={{ background: '#fff', borderRadius: 24, padding: 32, width: '100%', maxWidth: 440, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', animation: 'ord-in 0.3s ease' }}>
                        ...
                        <select className="ord-modal-sel" value={newStatus} onChange={e => setNewStatus(e.target.value)}
                            style={{ width: '100%', padding: '11px 14px', borderRadius: 10, border: '1.5px solid #e2e8f0', fontSize: 14, color: '#0f172a', background: '#f8fafc', cursor: 'pointer', marginBottom: 24, boxSizing: 'border-box', transition: 'all 0.2s' }}>
            ...
            {confirmDelete && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,12,41,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(4px)' }}
                    ...
                    <div style={{ background: '#fff', borderRadius: 24, padding: 32, width: '100%', maxWidth: 380, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', textAlign: 'center', animation: 'ord-in 0.3s ease' }}>
